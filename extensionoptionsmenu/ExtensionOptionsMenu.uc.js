...
// @note			以前zipに同梱していたcssは単体でロダに上げました
// @include			chrome://browser/content/browser.xul
// @author			hiyoko
// @version			ver2.5    ツールチップにxpiのサイズを表示するように
// ==/UserScript==

var EOM = {
// -----config-----
	TOOLBAR:			"toolbar-menubar",
	TARGET_BUTTON:		"null",			// ここで指定したIDの隣にボタンを配置 "null"で最後の位置に
		...
		AddonManager.getAllAddons(function(addon) {
			...
			addon.forEach(function(eAddons) {
				if (eAddons.type == "extension" && (!eAddons.appDisabled || (eAddons.appDisabled)) && ((eAddons.isActive && eAddons.optionsURL) || ((eAddons.userDisabled) || (!eAddons.userDisabled) || (eAddons.appDisabled)))) {
					...
					item.setAttribute("tooltiptext", eAddons.id + " " + "(" + "Größe: " + Math.floor(eAddons.size / 1024) + "KB" + ")");
