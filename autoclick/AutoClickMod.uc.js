...
(function(){
    ...
    var
        BUTTON_TYPE = 1,  // 0：In der Adressbar，1：frei zu positionieren，2：Menü
        ENABLE = true,
        hovering_time = 500,  // Einblendezeit in Millisekunden
    ...
    window.AutoClick = {
        ...
        rebuildButton: function() {
            ...
            if (BUTTON_TYPE === 2) {
                let menuitem = $C('menuitem', {
                    ...
                    tooltiptext: 'AutoClick Funktion',
        ...
        updateState: function(enable, isTmpDisable) {
            let preLabel = isTmpDisable ? 'Tempor\u00E4re' : '',
                label = 'AutoClick ist ' + preLabel + (enable ? 'aktiviert' : 'deaktiviert');
        ...
        openPref: function() {
            let xul = '<?xml version="1.0"?>\
                ...
                \
                    <hbox>\
                        <label value="Einblendezeit" />\
                        <textbox size="10" preference="hovering_time" />\
                        <label value="Millisekunden" />\
                    </hbox>\
                    <hbox>\
                        <label value="Typ der Schaltfl\u00E4che" />\
                        <menulist preference="BUTTON_TYPE">\
                          <menupopup>\
                            <menuitem label="Adressleiste" value="0"/>\
                            <menuitem label="Frei w\u00E4hlbar" value="1"/>\
                            <menuitem label="Men\u00FC" value="2"/>\
                          </menupopup>\
                        </menulist>\
                    </hbox>\
                    <label value="Klicken Sie auf den Knotennamen" />\
                    <textbox id="clicked_elem" flex="1" preference="clicked_elem" multiline="true" rows="3" />\
                    <label value="Brauchen Sie, um die Namen der \u00FCbergeordneten Elementknoten zu finden" />\
                    <textbox flex="1" id="clicked_parent_elem" preference="clicked_parent_elem" \
                        multiline="true" rows="3" />\
                    <button label="Zur\u00FCcksetzen" onclick="reset();" tooltiptext="Reset Top 2 wird auf den Standardwert festgelegt" />\
                \
                </prefpane>\
                <script>\
                    function reset() {\
